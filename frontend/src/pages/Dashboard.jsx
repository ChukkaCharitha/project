...
export default function Dashboard() {
  ...
  async function uploadFile(file) {
    ...
    const statuses = ['Uploading file...', 'Extracting text...', 'Splitting into chunks...', 'Building knowledge base...', 'Almost done...']
    let si = 0
    const iv = setInterval(() => {
      setUploadProgress(p => Math.min(p + 5, 88))
      si = Math.min(si + 1, statuses.length - 1)
      setUploadStatus(statuses[si])
    }, 400)
    ...
    try {
      ...
      setUploadStatus('')
      if (e.response?.status === 401) { localStorage.removeItem('token'); navigate('/login') }
      else toast.error(e.response?.data?.detail || 'Upload failed. Please try again.')
    }
  }
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
        ...
        .mesh{position:fixed;inset:0;pointer-events:none;z-index:0;background:${d ? 'radial-gradient(ellipse 80% 60% at 10% 10%,rgba(99,102,241,0.15) 0%,transparent 50%),radial-gradient(ellipse 60% 50% at 90% 90%,rgba(139,92,246,0.12) 0%,transparent 50%)' : 'radial-gradient(ellipse 80% 60% at 10% 10%,rgba(99,102,241,0.1) 0%,transparent 50%),radial-gradient(ellipse 60% 50% at 90% 90%,rgba(139,92,246,0.08) 0%,transparent 50%)'};}
        .grid-bg{position:fixed;inset:0;pointer-events:none;z-index:0;background-image:linear-gradient(${d ? 'rgba(99,102,241,0.05)' : 'rgba(99,102,241,0.08)'} 1px,transparent 1px),linear-gradient(90deg,${d ? 'rgba(99,102,241,0.05)' : 'rgba(99,102,241,0.08)'} 1px,transparent 1px);background-size:56px 56px;}
        .db{height:100vh;display:flex;flex-direction:column;color:${d ? '#e2e8f0' : '#1e293b'};position:relative;z-index:1;overflow:hidden;}
        .nav{height:60px;min-height:60px;flex-shrink:0;display:flex;align-items:center;justify-content:space-between;padding:0 28px;background:${d ? 'rgba(2,8,23,0.9)' : 'rgba(255,255,255,0.95)'};backdrop-filter:blur(24px);border-bottom:1px solid ${d ? 'rgba(99,102,241,0.12)' : 'rgba(99,102,241,0.15)'};}
        ...
        .bn{font-family:'Syne',sans-serif;font-size:18px;font-weight:800;color:${d ? '#fff' : '#0f172a'};}
        ...
        .nb{padding:8px 18px;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.25s;font-family:'DM Sans',sans-serif;}
        ...
        .pt{font-family:'Syne',sans-serif;font-size:28px;font-weight:800;letter-spacing:-1px;margin-bottom:4px;background:linear-gradient(135deg,${d ? '#fff' : '#0f172a'},#6366f1);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
        .ps{font-size:13px;color:${d ? '#475569' : '#64748b'};}
        .hbadge{padding:6px 14px;background:linear-gradient(135deg,rgba(99,102,241,0.15),rgba(139,92,246,0.1));border:1px solid rgba(99,102,241,0.3);border-radius:20px;font-size:11px;color:#818cf8;font-weight:700;animation:borderGlow 3s infinite;white-space:nowrap;}
        .stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;}
        .sc{background:${d ? 'rgba(15,23,42,0.7)' : 'rgba(255,255,255,0.9)'};border:1px solid ${d ? 'rgba(99,102,241,0.1)' : 'rgba(99,102,241,0.15)'};border-radius:16px;padding:18px;transition:all 0.3s;position:relative;overflow:hidden;}
        ...
        .sn{font-family:'Syne',sans-serif;font-size:36px;font-weight:800;background:linear-gradient(135deg,#6366f1,#8b5cf6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1;}
        .sl{font-size:12px;color:${d ? '#475569' : '#64748b'};margin-top:4px;font-weight:500;}
        ...
        .ut{font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:${d ? '#fff' : '#0f172a'};margin-bottom:5px;}
        .us{font-size:12px;color:${d ? '#475569' : '#64748b'};margin-bottom:12px;}
        .badges{display:flex;justify-content:center;gap:8px;}
        .badge{padding:4px 14px;background:rgba(99,102,241,0.1);border:1px solid rgba(99,102,241,0.25);border-radius:20px;font-size:11px;color:#818cf8;font-weight:700;}
        ...
        .sec-t{font-family:'Syne',sans-serif;font-size:15px;font-weight:700;color:${d ? '#e2e8f0' : '#1e293b'};}
        ...
        .dr:hover{border-color:rgba(99,102,241,0.35);transform:translateX(5px);}
        .dr:hover::before,.dr.active::before{transform:scaleY(1);}
        .dr.active{border-color:rgba(99,102,241,0.45);}
        .d-ico{font-size:20px;flex-shrink:0;}
        .d-info{flex:1;min-width:0;}
        .d-name{font-weight:600;color:${d ? '#e2e8f0' : '#1e293b'};font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px;}
        .d-meta{font-size:11px;color:${d ? '#334155' : '#94a3b8'};}
        .d-tb{font-size:9px;font-weight:800;color:#6366f1;background:rgba(99,102,241,0.1);padding:3px 8px;border-radius:5px;text-transform:uppercase;flex-shrink:0;}
        ...
        .e-t{font-family:'Syne',sans-serif;font-size:15px;font-weight:700;color:${d ? '#e2e8f0' : '#1e293b'};margin-bottom:6px;}
        .e-s{font-size:12px;color:${d ? '#475569' : '#64748b'};}
        .right{display:flex;flex-direction:column;overflow:hidden;background:${d ? 'rgba(8,14,30,0.6)' : 'rgba(248,250,252,0.9)'};}
        .ai-box{margin:18px 18px 0;background:linear-gradient(135deg,rgba(99,102,241,0.12),rgba(139,92,246,0.08));border:1px solid rgba(99,102,241,0.22);border-radius:16px;padding:18px;flex-shrink:0;}
        .ai-box-t{font-family:'Syne',sans-serif;font-size:14px;font-weight:800;color:${d ? '#c7d2fe' : '#4338ca'};margin-bottom:7px;}
        .ai-box-s{font-size:12px;color:${d ? '#64748b' : '#64748b'};line-height:1.6;margin-bottom:12px;}
        .ai-box-btn{width:100%;padding:11px;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none;border-radius:11px;font-size:13px;font-weight:700;cursor:pointer;transition:all 0.3s;font-family:'DM Sans',sans-serif;box-shadow:0 4px 14px rgba(99,102,241,0.35);}
        ...
        .qcard:hover{border-color:rgba(99,102,241,0.22);}
        .qcard-t{font-size:10px;font-weight:800;color:${d ? '#334155' : '#94a3b8'};text-transform:uppercase;letter-spacing:1.2px;margin-bottom:10px;}
        .qcard-body{flex:1;display:flex;flex-direction:column;gap:7px;overflow:hidden;}
        .act{display:flex;align-items:center;gap:8px;padding:8px 10px;background:${d ? 'rgba(15,23,42,0.5)' : 'rgba(255,255,255,0.7)'};border:1px solid ${d ? 'rgba(99,102,241,0.06)' : 'rgba(99,102,241,0.1)'};border-radius:10px;transition:all 0.25s;flex-shrink:0;}
        ...
        .act-t{font-size:11px;font-weight:500;color:${d ? '#94a3b8' : '#475569'};flex:1;}
        .act-tm{font-size:10px;color:${d ? '#334155' : '#94a3b8'};flex-shrink:0;}
        .qa-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;flex:1;}
        .qa{padding:0 8px;border-radius:10px;border:1px solid ${d ? 'rgba(99,102,241,0.1)' : 'rgba(99,102,241,0.15)'};background:${d ? 'rgba(15,23,42,0.5)' : 'rgba(255,255,255,0.7)'};cursor:pointer;transition:all 0.25s;font-family:'DM Sans',sans-serif;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;}
        .qa:hover{border-color:rgba(99,102,241,0.4);transform:translateY(-3px);}
        .qa-i{font-size:20px;}
        .qa-l{font-size:11px;font-weight:600;color:${d ? '#94a3b8' : '#475569'};}
        .tip{display:flex;gap:10px;padding:10px 12px;background:${d ? 'rgba(15,23,42,0.5)' : 'rgba(255,255,255,0.7)'};border:1px solid ${d ? 'rgba(99,102,241,0.06)' : 'rgba(99,102,241,0.1)'};border-radius:10px;transition:all 0.25s;flex-shrink:0;}
        .tip:hover{border-color:rgba(99,102,241,0.25);transform:translateY(-2px);}
        .tip-i{width:32px;height:32px;border-radius:9px;background:rgba(99,102,241,0.1);display:flex;align-items:center;justify-content:center;font-size:15px;flex-shrink:0;}
        .tip-t{font-size:12px;font-weight:700;color:${d ? '#c7d2fe' : '#4338ca'};margin-bottom:2px;}
        .tip-s{font-size:11px;color:${d ? '#475569' : '#64748b'};line-height:1.4;}
        ...
        .usage-label{font-size:11px;color:${d ? '#64748b' : '#64748b'};font-weight:500;}
        ...
        .det-wrap{display:flex;flex-direction:column;height:100%;padding:18px;gap:14px;}
        ...
        .det-name{font-family:'Syne',sans-serif;font-size:15px;font-weight:800;color:${d ? '#fff' : '#0f172a'};margin-bottom:5px;word-break:break-all;}
        .det-date{font-size:11px;color:${d ? '#475569' : '#64748b'};}
        ...
        .det-p:hover{transform:translateY(-2px);}
      ...
      {particles.map(p => (
        <div key={p.id} style={{ position:'fixed', width:p.size, height:p.size, borderRadius:'50%', background:p.color, left:`${p.x}%`, bottom:'-10px', opacity:0, animation:`floatParticle ${p.duration}s ${p.delay}s infinite linear`, pointerEvents:'none', zIndex:0, boxShadow:`0 0 ${p.size*2}px ${p.color}` }} />
      ))}
          ...
          <div className="left">
            <div className="ph">
              <div>
                ...
                <p className="ps">Upload documents · AI answers instantly</p>
              </div>
            ...
            <div className="stats">
              {[
                ...
                <div className="sc" key={i}>
                  <div className="sc-top"><span className="sc-ico">{s.ico}</span><span className="sc-badge">{s.badge}</span></div>
            ...
            <div className="uz-wrap">
              <div className={`uz ${dragOver ? 'drag' : ''}`}
                ...
                {uploading ? (
                  <>
                    <div className="uspin" />
                    <div className="ut">{uploadStatus}</div>
                    <div className="us">Please wait while we process your document</div>
                    <div className="prog-wrap">
                      <div className="prog-label"><span>Processing</span><span>{uploadProgress}%</span></div>
            ...
            <div className="search-wrap">
              ...
              <input className="search-inp" placeholder="Search your documents..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            ...
            {selected ? (
              ...
              <>
                <div className="ai-box">
                  ...
                  <div className="ai-box-s">Upload multiple PDFs and text files. I'll search across ALL documents to give you the most accurate answers.</div>
                ...
                <div className="rp-grid">
                  <div className="qcard">
                    ...
                    <div className="qcard-body">
                      {[
                        { ico:'🤖', t:'AI ready to answer', tm:'Now', c:'#6366f1' },
                        { ico:'📚', t:`${documents.length} docs loaded`, tm:'Active', c:'#10b981' },
                        { ico:'⚡', t:'Groq LLaMA 3.3', tm:'Online', c:'#f59e0b' },
                      ].map((a,i) => (
                    ...
                    <div className="qcard-body">
                      <div className="usage-box">
                        <div className="usage-row">
                          <span className="usage-label">Documents used</span>
                          <span className="usage-val">{documents.length}/20</span>
                        </div>
                        <div className="usage-track">
                          <div className="usage-fill" style={{ width:`${Math.min((documents.length/20)*100,100)}%` }} />
                        </div>
                      </div>
                      {[
                        { i:'💡', t:'Natural language', s:'Ask questions naturally!' },
                        { i:'⚡', t:'LLaMA 3.3 70B', s:'Ultra-fast AI responses' },
                      ].map((t,i) => (
                        <div className="tip" key={i}>
                          <div className="tip-i">{t.i}</div>
                          <div><div className="tip-t">{t.t}</div><div className="tip-s">{t.s}</div></div>
                ...
                <div className="rp-footer">
                  <div className="status-bar">
                    <div className="status-left"><span>⚡</span><span>Groq LLaMA 3.3 · Free tier</span></div>
